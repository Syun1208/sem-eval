...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("preprocess.log", mode="a", encoding="utf-8")
    ]
)
...
def process_data_split(input_path: str, output_path: str, extractor: AgnosticFeatureExtractor):
    ...
    for code in tqdm(df['code'], desc="Processing", dynamic_ncols=True):
        ...
        try:
            ...
            logger.warning(f"Error processing snippet {i}: {e}. Using zero-vector.")
...
def run_split(name, input_path, output_path, extractor):
    logger.info(f"--- Starting {name} set processing ---")
    process_data_split(input_path, output_path, extractor)
    logger.info(f"--- Finished {name} set processing ---")
...
if __name__ == "__main__":
    ...
    # 2. Setup Device
    device_train = f"cuda:5" if torch.cuda.is_available() else "cpu"
    device_val = f"cuda:7" if torch.cuda.is_available() else "cpu"
    logger.info(f"Using device: {device_train} and {device_val}")

    # 3. Initialize Extractor
    train_extractor = AgnosticFeatureExtractor(config, device_train)
    val_extractor = AgnosticFeatureExtractor(config, device_val)
    ...
    train_output = os.path.join(proc_dir, f"train_processed_{perplexity_model_name_save}.parquet")
    val_output = os.path.join(proc_dir, f"val_processed_{perplexity_model_name_save}.parquet")

    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = [
            executor.submit(run_split, "TRAIN", train_input, train_output, train_extractor),
            executor.submit(run_split, "VAL", val_input, val_output, val_extractor),
        ]
        ...
        for future in as_completed(futures):
            try:
                ...
                logger.exception(f"Error in split processing: {e}")
